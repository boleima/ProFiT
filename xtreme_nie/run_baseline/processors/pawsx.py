...
class PawsxProcessor(DataProcessor):
        ...
        if num_sample != -1:
            ...
            for example in examples:
                if example.label==labels[0] and len(l0)<num_sample:
                    ...
                    break
            examples = l0+l1
        print(f"number of training examples is: {len(examples)}")
        return examples

    def get_translate_examples(self, data_dir, language='en', split='train', num_sample=-1):
        """See base class."""
    ...
    def get_translate_train_examples(self, data_dir, language='en', num_sample=-1):
        """See base class."""
        return self.get_translate_examples(data_dir, language, split='train', num_sample=num_sample)

    def get_translate_test_examples(self, data_dir, language='en', num_sample=-1):
        """See base class."""
        return self.get_translate_examples(data_dir, language, split='test', num_sample=num_sample)
