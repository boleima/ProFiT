...
def evaluate(args, model, tokenizer, split='train', language='en', lang2id=None, prefix="", output_file=None, label_list=None, output_only_prediction=True):
  ...
  for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
    eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, mode = 'eval', split=split, language=language, lang2id=lang2id, evaluate=True)
...
def load_and_cache_examples(args, task, tokenizer, mode, split='train', language='en', lang2id=None, evaluate=False):
...
def main():
  ...
  if args.do_train:
    ...
    train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, mode = 'train', split=args.train_split, language=args.train_language, lang2id=lang2id, evaluate=False)
