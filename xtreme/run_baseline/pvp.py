...
from run_baseline.processors.utils import InputExample, get_verbalization_ids
...
class XnliPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        ('Issues in Data Synthesis .', True)
        Problems in data synthesis .
        ('Problems in data synthesis .', True)
        ('Issues in Data Synthesis .', True) . [MASK] ('Problems in data synthesis .', True)
            """
...
# ['"', (['You have access to the facts .'], True), '" ?'] ['[MASK]', ', "', ('The facts are accessible to you .', True), '"']
...
class PawsxPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return ['"', text_a, '" / "', text_b, ' Similar sense?', self.mask, '.'], []
        if self.pattern_id == 1:
            return ['Do sentence',text_a, "and sentence", text_b, ' have the same meaning?', self.mask, '.'], []
        if self.pattern_id == 2:
            return [text_a, text_b, "The previous two sentences are paraphrases. Right?", self.mask, '.'], []
